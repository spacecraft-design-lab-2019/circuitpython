...
mp_obj_t ulab_linalg_dot(mp_obj_t _m1, mp_obj_t _m2) {
    // TODO: should the results be upcast?
    ulab_ndarray_obj_t *m1 = MP_OBJ_TO_PTR(_m1);
    ulab_ndarray_obj_t *m2 = MP_OBJ_TO_PTR(_m2);    
    if(m1->n != m2->m) {
        mp_raise_ValueError(translate("matrix dimensions do not match"));
    }
    // TODO: numpy uses upcasting here
    ulab_ndarray_obj_t *out = create_new_ndarray(m1->m, m2->n, NDARRAY_FLOAT);
    ...
    for(size_t i=0; i < m1->m; i++) { // rows of m1
        for(size_t j=0; j < m2->n; j++) { // columns of m2
            ...
            for(size_t k=0; k < m2->m; k++) {
                // (i, k) * (k, j)
                v1 = ulab_ndarray_get_float_value(m1->array->items, m1->array->typecode, i*m1->n+k);
                v2 = ulab_ndarray_get_float_value(m2->array->items, m2->array->typecode, k*m2->n+j);
                sum += v1 * v2;
            }
...
mp_obj_t ulab_linalg_cross(mp_obj_t _m1, mp_obj_t _m2) {
    // TODO: should the results be upcast?
    ulab_ndarray_obj_t *m1 = MP_OBJ_TO_PTR(_m1);
    ulab_ndarray_obj_t *m2 = MP_OBJ_TO_PTR(_m2);    
    if((m1->n != 3) || (m2->n != 3)) {
        mp_raise_ValueError(translate("vectors are not the right size"));
    }
    ulab_ndarray_obj_t *out = create_new_ndarray(m1->m, m2->n, NDARRAY_FLOAT);
    mp_float_t *outdata = (mp_float_t *)out->array->items;
    mp_float_t a1, a2, a3, b1, b2, b3;

    //get the values out of the arrays
    size_t index1=0;
    a1 = ulab_ndarray_get_float_value(m1->array->items, m1->array->typecode, index1);
    b1 = ulab_ndarray_get_float_value(m2->array->items, m2->array->typecode, index1);
    size_t index2=1;
    a2 = ulab_ndarray_get_float_value(m1->array->items, m1->array->typecode, index2);
    b2 = ulab_ndarray_get_float_value(m2->array->items, m2->array->typecode, index2);
    size_t index3=2;
    a3 = ulab_ndarray_get_float_value(m1->array->items, m1->array->typecode, index3);
    b3 = ulab_ndarray_get_float_value(m2->array->items, m2->array->typecode, index3);

    // perform the cross product
    outdata[index1] = a2*b3-a3*b2;
    outdata[index2] = a3*b1-a1*b3;
    outdata[index3] = a1*b2-a2*b1;

    return MP_OBJ_FROM_PTR(out);
}


mp_obj_t ulab_linalg_zeros_ones(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args, uint8_t kind) {
    static const mp_arg_t allowed_args[] = {
    ...
    ulab_ndarray_obj_t *ndarray = NULL;
...
mp_obj_t ulab_linalg_zeros(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    return ulab_linalg_zeros_ones(n_args, pos_args, kw_args, 0);
}

mp_obj_t ulab_linalg_ones(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    return ulab_linalg_zeros_ones(n_args, pos_args, kw_args, 1);
}

mp_obj_t ulab_linalg_eye(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    static const mp_arg_t allowed_args[] = {
    ...
    ulab_ndarray_obj_t *ndarray = create_new_ndarray(m, n, dtype);
    mp_obj_t one = mp_obj_new_int(1);
    size_t i = 0;
    if((k >= 0) && (abs(k) < n)) {
        while(abs(k) < n) {
...
mp_obj_t ulab_linalg_det(mp_obj_t oin) {
    if(!MP_OBJ_IS_TYPE(oin, &ulab_ndarray_type)) {
        mp_raise_TypeError(translate("function defined for ndarrays only"));
    }
    ulab_ndarray_obj_t *in = MP_OBJ_TO_PTR(oin);
    if(in->m != in->n) {
    ...
    for(size_t i=0; i < in->array->len; i++){
        tmp[i] = ulab_ndarray_get_float_value(in->array->items, in->array->typecode, i);
    }
    mp_float_t c;
...
mp_obj_t ulab_linalg_eig(mp_obj_t oin) {
    if(!MP_OBJ_IS_TYPE(oin, &ulab_ndarray_type)) {
        mp_raise_TypeError(translate("function defined for ndarrays only"));
    }
    ulab_ndarray_obj_t *in = MP_OBJ_TO_PTR(oin);
    ...
    for(size_t i=0; i < in->array->len; i++) {
        array[i] = ulab_ndarray_get_float_value(in->array->items, in->array->typecode, i);
    }
    ...
    ulab_ndarray_obj_t *eigenvectors = create_new_ndarray(in->m, in->n, NDARRAY_FLOAT);
    ...
    ulab_ndarray_obj_t *eigenvalues = create_new_ndarray(1, in->n, NDARRAY_FLOAT);
